# source : https://github.com/thnkim/OpenFacePytorch/blob/master
import sys
import numpy
import torch
...
def load_openface(useCuda=True, gpuDevice=0, useMultiGPU=False, weights: str='openface_cpu_weights.pth'):
    model = netOpenFace(useCuda, gpuDevice)
    model.load_state_dict(torch.load(os.path.join(containing_dir, weights), map_location="cpu"))
